...
export const getKitFileBuffer = (kit, pads) => {
  ...
  KitBuffer.PAD_FILE_ORDER.forEach((padType) => {
    if (padType === KitBuffer.DIVIDER_PAD_TYPE) {
      // need to insert the divider
      buffer = buffer.concat(getPadBlock1Divider());
    } else {
  ...
  KitBuffer.PAD_FILE_ORDER.forEach((padType) => {
    if (padType === KitBuffer.DIVIDER_PAD_TYPE) {
      // need to insert the divider
      buffer = buffer.concat(getPadBlock2Divider());
    } else {
      let pad = getPadWithType(kit, pads, padType);
